...
const allUser = async (query: Record<string, unknown>) => {
     ...
     const users = await queryBuilder.search(['firstName', 'lastName', 'phone', 'email', 'address']).filter().sort().paginate().fields().modelQuery.exec();
...
const deleteUser = async (id: string) => {
     const result = await User.findByIdAndUpdate(id, { $set: { isDeleted: true } }, { new: true });
     if (!result) {
          throw new AppError(StatusCodes.NOT_FOUND, 'User not found.');
     }
     return result;
